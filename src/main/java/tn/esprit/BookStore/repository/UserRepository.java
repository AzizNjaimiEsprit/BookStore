...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    User Auth(@Param("login") String login);

    @Query("SELECT u FROM User u WHERE u.role= 1")
    List<User> WhereAdmin();

    @Query("select case when count(u) > 0 then true else false end from User u WHERE u.login= :login AND u.password = :password")
    boolean existence(@Param("login") String login, @Param("password") String pwd);
    ...
    boolean Existmail(@Param("email") String email, @Param("login") String login);
}
